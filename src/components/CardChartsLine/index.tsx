      ...
      <Flex pt={2} px={6} flexWrap="wrap">
        <Flex flexDir="column" flex="1" className="balance-l-c">
          <Text
            ...
            lineHeight="2.3rem"
          >
            ${chardata?.info?.current_price}
          </Text>
          {chardata?.info?.price_change_percentage_24h > 0 ? (
            <Text color="#098551">
              <Highlight query="All time" styles={{ color: "#000" }}>
                {`↗ ${formatPercent(
                  chardata?.info?.price_change_percentage_24h / 100
                )} All time`}
              </Highlight>
            </Text>
          ) : (
            <Text color="#f00">
              <Highlight query="All time" styles={{ color: "#000" }}>
                {`↙ ${formatPercent(
                  chardata?.info?.price_change_percentage_24h
                )} All time`}
              </Highlight>
            </Text>
          )}
